...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("infoblox-fqdn-regex", "Apply this regular expression as a filter for obtaining zone_auth objects (optional).").Default(defaultConfig.InfobloxFQDNRegEx).StringVar(&cfg.InfobloxFQDNRegEx)
	app.Flag("infoblox-name-regex", "Apply this regular expression as a filter for obtaining name record objects (optional).").Default(defaultConfig.InfobloxNameRegEx).StringVar(&cfg.InfobloxNameRegEx)
